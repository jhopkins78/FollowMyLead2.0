...
class LeadScoringService:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._model = LeadScorer()
            # Try to load training data and train model
            try:
                with open('mock_data.json', 'r') as f:
                    training_data = json.load(f)
                    cls._model.train(training_data)
                    current_app.logger.info("Lead scoring model trained successfully")
            except Exception as e:
                current_app.logger.warning(f"Could not train lead scoring model: {str(e)}")
